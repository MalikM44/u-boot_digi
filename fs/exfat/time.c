		      ...
		      uint8_t *centisec, uint8_t *tzoffset)
{
	...
	u32 shift = EPOCH_DIFF_SEC + exfat_timezone;
	...
	/* rem contains seconds within the day */
	hour = rem / SEC_IN_HOUR;
	rem %= SEC_IN_HOUR;
	...
	*time = cpu_to_le16(twosec | (min << 5) | (hour << 11));
	...
	if (centisec)
		*centisec = (rem & 1) ? 100 : 0;

	/* record local timezone offset (15-minute increments) */
